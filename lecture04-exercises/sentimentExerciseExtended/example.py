...
testudf = udf(lambda s: wordSentiment(s))

test = words.withColumn("Sentiment", testudf(col("word")))

# Write it back to Kafka
test.select(test.Sentiment).alias('value').selectExpr("CAST(value AS STRING)").writeStream \
    .format('kafka') \
    .option("kafka.bootstrap.servers", "kafka:9092") \
